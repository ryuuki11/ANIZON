...
if(isset($_SESSION['member']['m_name'])){
    if(isset($_POST['kazu'])&& $_POST['kazu']!=""){
    $_SESSION['gacha']['num']=$_POST['kazu'];
    }else if(!isset($_SESSION['gacha']['num'])){
    ...
    echo '<div class="card">';
        ...
        echo'</div>';
        echo'<div>';
        echo '<p class="allprice">合計 ',$_SESSION['gacha']['num']*$_SESSION['gacha']['price'],'円</p>';
        
    echo '</div>';
    
        $_SESSION['flag']=1;
        echo '<div class="gachab"><button type="submit">購入</button></div>';
        echo '</form>';
    }else{
